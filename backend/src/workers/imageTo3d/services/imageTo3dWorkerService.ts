...
export class ImageTo3dWorkerService {
  /**
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
          throw new Error('Lightning AI service request timed out after 5 minutes');
        }
  ...
  private static async processWithFallback(job: Job<AiJobData>, imageBuffer: Buffer) {
    const { designName, ownerId, correlationId, mimeType } = job.data;
    ...
    try {
      ...
      // 2. Fallback: Use the local 3dMock.stl mock file
      try {
        const mockPath = path.join(process.cwd(), "../tmp/3d/3dMock.stl");
